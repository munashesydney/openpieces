...
import { LineChart, RefreshCw, AlertCircle, ChevronLeft, ChevronRight } from "lucide-react";
import { DateRangePicker } from "@/components/ui/date-range-picker";
import Link from "next/link";

export function UsageClient({ workspaceId, ordId }: { workspaceId: string, ordId: string }) {
  const [isPending, startTransition] = useTransition();
  const [data, setData] = useState<{ metrics: any, records: any[], total: number } | null>(null);
  ...
  const loadData = () => {
    startTransition(async () => {
      setError(null);
      const res = await getUsageDataAction(workspaceId, page, pageSize, startDate || undefined, endDate || undefined);
      if (res.error) {
        setError(res.error);
      } else if (res.metrics && res.records) {
        setData({ metrics: res.metrics, records: res.records, total: res.total || 0 });
      }
    });
  };
