...
export function OpenCodePageSkeleton() {
  return (
    <div className="flex h-full min-h-[320px] text-sm animate-pulse">
      <div className="flex h-full w-full lg:w-64 shrink-0 flex-col border-r border-[var(--border)] bg-[var(--background)]">
        ...
        <div className="flex-1 space-y-2 overflow-hidden p-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-11 w-full rounded-md bg-[var(--hover-bg)]"
            />
          ))}
        </div>
      </div>

      <div className="hidden lg:flex min-w-0 flex-1 flex-col bg-[var(--background)]">
